...
public:
   ...
   */
  void SwitchToTx (Time txDuration, Ptr<const Packet> packet, double txPowerDbm, WifiTxVector txVector,Mac48Address address);/////liang
  void SwitchToRx (Time rxDuration,Mac48Address address);
  void SwitchFromRxEndOk (Ptr<Packet> packet, double snr, WifiTxVector txVector,Mac48Address address);
  void SwitchFromRxBecauseWrongAddress(void);////liang
  void SwitchFromRxEndError (Ptr<Packet> packet, double snr,Mac48Address address);
  void SwitchMaybeToCcaBusy (Time duration,Mac48Address address);
  ...
  void NotifyBusytoneTxPowerReady(Time rxDuration);/////liang
  void NotifyToleranceInterference(double ToleranceInterference, WifiTxVector tx_vector,Time duration,ns3::WifiMacHeader hdr);////從wifiPhy::StartReceivePacket() 得到還可以忍受的的干擾

  void NotifyToleranceInterference(double ToleranceInterference);////從wifiPhy::StartReceivePacket() 得到還可以忍受的的干擾
  bool IsReservedForAck();
  bool IsReservedForCts();
  void HasReserveForAck(bool flag);
  void HasReserveForCts(bool flag);
private:
  /**
   ...
   * \param txPowerDbm the nominal tx power in dBm
   */
  void NotifyRxStart1(Time duration,Mac48Address address);
  void NotifyRxEndOk1(Mac48Address address);
  void NotifyRxEndError1(Mac48Address address);
  void NotifyTxStart1(Time duration,Mac48Address address);
  void NotifyMaybeCcaBusyStart1(Time duration,Mac48Address address);
  void NotifyRxEndErrorBecauseWrongAddress(void);  ////liang
  ...
  bool m_reserveforack;
  bool m_reserveforcts;
