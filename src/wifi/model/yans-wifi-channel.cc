...
//std::cout<<GetDevice(sender->GetDevice()->GetNode ()->GetId ())->GetAddress();
//std::cout<<" No."<<sender->GetDevice()->GetNode ()->GetId ()<<" device send rx duration is "<<duration.GetMicroSeconds()<<" "<<txPowerDbm<<" at "<<Simulator::Now().GetMicroSeconds()<<std::endl;
...
{///std::cout<<"No."<<phy->GetDevice()->GetNode ()->GetId ()<<" device rx at "<<Simulator::Now().GetMicroSeconds()<<" rxpower is "<<rxPowerDbm<<std::endl;
...
double //////liang
YansWifiChannel::GetOthersTxpowerW(Ptr<YansWifiPhy> receiver)
{
       double rxPowerW = 0;
       ...
       for (PhyList::const_iterator i = m_phyList.begin (); i != m_phyList.end (); i++)
       {
        if (receiver != (*i))
        {
          ...
          //std::cout<<(*i)->GetDevice()->GetNode ()->GetId ()<<" "<<txPowerDbm<<" "<<DbmToW(rxPowerDbm)<<std::endl;
        }
       }
