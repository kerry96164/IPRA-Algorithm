...
WifiNetDevice::CompleteConfig (void)
{
  if (m_mac == 0
      ...
      || m_stationManager == 0
      || m_node == 0
      || m_configComplete || m_busytonephy == 0)///////liang  m_busytonephy要先給0嗎
    {
      return;
    }
  m_mac->SetWifiRemoteStationManager (m_stationManager);
  m_mac->SetWifiPhy (m_phy,m_busytonephy);
  //m_mac->SetBusytonePhy(m_busytonephy);////////////liang
...
WifiNetDevice::GetBusytonePhy()
{
	return m_busytonephy;
}

void ////////liang
WifiNetDevice::SetBusytonePhy (const Ptr<BusytonePhy> btphy)
{
	m_busytonephy = btphy;
	CompleteConfig();
}
