...
public:
  ...
  void NotifyRxEndErrorBecauseWrongAddress(void)
  {
    m_dcf->NotifyRxEndErrorBecauseWrongAddressNow();//////////liang
  }
...
{//std::cout<<"request "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this << state);
  ...
  UpdateBackoff ();
  //NS_ASSERT (!state->IsAccessRequested ());////liang
  ...
  if (state->GetBackoffSlots () == 0)
    {
      if (IsBusy ())
        ...
        {//std::cout<<" aifs "<<Simulator::Now().GetMicroSeconds()<<std::endl;
          NS_LOG_DEBUG ("busy within AIFS");
          //Simulator::Schedule(MicroSeconds(27), &DcfManager::RequestAccess, this,state);
          state->NotifyCollision ();
          DoRestartAccessTimeoutIfNeeded ();////liang
          return;
        }
    }
...
{//std::cout<<"DoGrantAccess "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
  uint32_t k = 0;
  for (States::iterator i = m_states.begin (); i != m_states.end (); k++)
    {
     Ptr<DcfState> state = *i;
//std::cout<<GetBackoffEndFor (state).GetMicroSeconds()<<" "<<Simulator::Now().GetMicroSeconds()<<std::endl;
...
{//std::cout<<"AccessTimeout "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
  UpdateBackoff ();
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
...
///std::cout<<"rx "<<rxAccessStart.GetMicroSeconds()<<" rxstart "<<m_lastRxStart.GetMicroSeconds()<<" rxduration "<<m_lastRxDuration.GetMicroSeconds()<<std::endl;
...
{//std::cout<<"update"<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
  uint32_t k = 0;
  for (States::iterator i = m_states.begin (); i != m_states.end (); i++, k++)
    {
      Ptr<DcfState> state = *i;
///////std::cout<<GetBackoffStartFor (state).GetMicroSeconds()<<" "<<Simulator::Now().GetMicroSeconds()<<std::endl;
...
{//std::cout<<"dorestart"<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
  /**
...
{//std::cout<<" rxstart "<<Simulator::Now().GetMicroSeconds()<<std::endl;
...
{//std::cout<<" rxend "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
...
{//std::cout<<" rxenderror "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
...
void//////////////////liang
DcfManager::NotifyRxEndErrorBecauseWrongAddressNow(void)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("rx end ok");
  m_lastRxEnd = Simulator::Now ();
...
{//std::cout<<" txstart "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this << duration);
  if (m_rxing)
    {
     ...
     //std::cout<<m_lastRxStart.GetMicroSeconds()<<" "<<Simulator::Now().GetMicroSeconds()<<m_sifs.GetMicroSeconds()<<std::endl;
