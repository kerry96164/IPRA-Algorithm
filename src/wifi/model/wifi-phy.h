...
public:
  ...
  virtual void NotifyBusytoneTxPowerReady(Time rxDuration){};/////liang
  virtual void NotifyToleranceInterference(double ToleranceInterference, WifiTxVector tx_vector,Time duration,ns3::WifiMacHeader hdr){};////從WifiPhyStateHelper::NotifyToleranceInterference() 得到還可以忍受的的干擾
  ...
  virtual void NotifyMaybeCcaBusyStart1(Time duration, Mac48Address){};
  //////////////////////////////////////////////////////////////////////////////////////////////
  /**
...
public:
  ...
  class Transmitpower
    {
      ...
      ~Transmitpower(){}
      Transmitpower(double power,Time durationofpower,double gain_ofpacket)
      {
        powerw = power;
        duration = durationofpower;
        gain = gain_ofpacket;
      }
  ...
  class Position
  {
    public:
      ...
      ~Position(){}
      Position(double x_new, double y_new, double z_new)
      {
        x = x_new;
        y = y_new;
        z = z_new;
  ...
  void AdjustRate(bool flag);
  
   WifiMode ChooseRate(uint32_t mcs);
  /**
  ...
  void ReceiveWrongPacket(void);///////////////////liang
  double local_gain = 0.00000001;
  ...
  virtual double GetOthersTxpowerW();////liang
  
  void SetTxPowerDbm(double txpower,Time duration);
  void Turnoff();
  double GetSNR(double rxPowerW, uint8_t channelWidth);////liang
  std::vector<Mac48Address>m_addressregister;
  void ResetAddressRegister();
  ...
  /** 802.11ah
   * \return the WifiMode used for the transmission of the S1G-STF, S1G-LTF and S1G-SIG-B fields
   */
  static WifiMode GetS1gPlcpHeaderMode ();
  /**
  ...
  */
   ...
   * \return the MCS index whose index is specified.
   */
  WifiMode GetMcs (int mcs) const;

  /**
   * \brief Set channel number.
   *
   * Channel center frequency = Channel starting frequency + 5 MHz * (nch - 1) in non-S1G bands
   * Channel center frequency = Channel starting frequency + 0.5 MHz * nch in S1G bands
   *
  ...
  /**
   * Return MCS 10 from VHT MCS values, for 802.11ah.
   *
   * \return MCS 10 from VHT MCS values
   */
  // static WifiMode GetVhtMcs10 (); // Jonathan

  //802.11ah
  /**
   * Return MCS 0 from S1G MCS values.
   *
   * \return MCS 0 from S1G MCS values
   */
  static WifiMode GetS1gMcs0 ();
  /**
   * Return MCS 1 from S1G MCS values.
   *
   * \return MCS 1 from S1G MCS values
   */
  static WifiMode GetS1gMcs1 ();
  /**
   * Return MCS 2 from S1G MCS values.
   *
   * \return MCS 2 from S1G MCS values
   */
  static WifiMode GetS1gMcs2 ();
  /**
   * Return MCS 3 from S1G MCS values.
   *
   * \return MCS 3 from S1G MCS values
   */
  static WifiMode GetS1gMcs3 ();
  /**
   * Return MCS 4 from S1G MCS values.
   *
   * \return MCS 4 from S1G MCS values
   */
  static WifiMode GetS1gMcs4 ();
  /**
   * Return MCS 5 from S1G MCS values.
   *
   * \return MCS 5 from S1G MCS values
   */
  static WifiMode GetS1gMcs5 ();
  /**
   * Return MCS 6 from S1G MCS values.
   *
   * \return MCS 6 from S1G MCS values
   */
  static WifiMode GetS1gMcs6 ();
  /**
   * Return MCS 7 from S1G MCS values.
   *
   * \return MCS 7 from S1G MCS values
   */
  static WifiMode GetS1gMcs7 ();
  /**
   * Return MCS 8 from S1G MCS values.
   *
   * \return MCS 8 from S1G MCS values
   */
  static WifiMode GetS1gMcs8 ();
  /**
   * Return MCS 9 from S1G MCS values.
   *
   * \return MCS 9 from S1G MCS values
   */
  static WifiMode GetS1gMcs9 ();
  /**
   * Return MCS 10 from S1G MCS values.
   *
   * \return MCS 10 from S1G MCS values
   */
  static WifiMode GetS1gMcs10 ();
  ...
  /**
   * Enable or disable support for HT/VHT/S1G short guard interval.
   *
  ...
  /** 802.11ah
   * Enable or disable S1g1Mfield support.
   *
  ...
  /**802.11ah
   * Return whether S1g1Mfield is supported.
   *
  ...
  Ptr<WifiPhyStateHelper> GetWifiPhyState();
...
protected:
  ...
  EventId m_endRxWrongEvent;
  EventId deletepowerevent;
  void Resetpowerlist(void);
private:
  /**
  ...
  void MaybeCcaBusyDuration (Mac48Address address);
