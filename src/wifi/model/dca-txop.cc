...
uint32_t
DcaTxop::GetRandomNumber(uint32_t min,uint32_t max)
{
        uint32_t x;
        x =rand()%max+1;
        return x;
...
{std::cout<<"start1"<<std::endl;
  ...
  if ((m_currentPacket != 0
    ...
    {////liang
	  ...
	  //else
	  //Simulator::Schedule (MicroSeconds(20), &EdcaTxopN::RestartAccessIfNeeded, this); /////why 20 ms?
    }
}
...
void /////liang
DcaTxop::ChannelBusy(void)
{std::cout<<"busy1"<<std::endl;
...
{std::cout<<"startnextfragment "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
