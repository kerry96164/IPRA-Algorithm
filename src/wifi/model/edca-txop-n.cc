...
{//std::cout<<"node "<<GetLow()->GetPhy()->GetDevice()->GetNode ()->GetId ()<<" backoff "<<m_dcf->GetBackoffSlots()<<" "<<Simulator::Now().GetMicroSeconds()<<std::endl;
if(GetLow()->GetPhy()->GetWifiPhyState()->IsStateTx())
  {//std::cout<<"node "<<GetLow()->GetPhy()->GetDevice()->GetNode ()->GetId ()<<" restart "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  ...
  m_startTxop = Simulator::Now ();
   m_mpduAggregator ->SetMaxAmpduSize(0);
  ...
  if (m_currentHdr.GetAddr1 ().IsGroup ())
    {
      ...
      m_currentParams.DisableNextData ();
          //std::cout<<"group "<<m_currentPacket->GetSize()<<std::endl;
          ...
          if (IsLastFragment ())
          ...
          else
            {
              ...
              m_currentParams.EnableNextData (GetNextFragmentSize ());
                     ...
                     //std::cout<<"2 "<<m_currentPacket->GetSize()<<std::endl;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
             ...
             {
             int currentPacketSize = 1508;             
             int cw;
             cw=log2(m_dcf->GetCw()+1)-4;
             ///std::cout<<"cw = "<<cw<<" "<<Simulator::Now().GetMicroSeconds()<<" "<<std::endl;
             switch (cw)
                 {
                case 0:
                        m_msduAggregator->SetMaxAmsduSize(currentPacketSize);
                        //std::cout<<m_currentPacket->GetSize()<<" ";
                        break;
                case 1:
                        m_msduAggregator->SetMaxAmsduSize((currentPacketSize+14+2)*(cw+1)-2);
                        //std::cout<<(currentPacketSize+14+2)*(cw+1)-2<<" ";
                        break;
                case 2:
                        m_msduAggregator->SetMaxAmsduSize((currentPacketSize+14+2)*(cw+1)-2);
                        //std::cout<<(currentPacketSize+14+2)*(cw+1)-2<<" ";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////            
          ...
          m_currentParams.DisableNextData ();
          //std::cout<<"3 "<<m_currentPacket->GetSize()<<std::endl;
          m_low->StartTransmission (m_currentPacket, &m_currentHdr, m_currentParams, this);
...
EdcaTxopN::NotifyCollision (void)
{
 ...
 if(Simulator::Now().GetSeconds() < starttime)
  ...
  {
    //m_backoffTrace = m_rng->GetInteger (0, m_dcf->GetCw ());
    m_backoffTrace = GetRandomNumber (0, m_dcf->GetCw ());
  }
  m_dcf->StartBackoffNow (m_backoffTrace);
  RestartAccessIfNeeded ();
}
...
{///std::cout<<"gotack "<<Simulator::Now().GetSeconds()<<std::endl;
if(Simulator::Now().GetSeconds() > starttime)
{
  NS_LOG_FUNCTION (this);
      ...
      if (!HasTxop ())
        {
          ...
          m_cwTrace = m_dcf->GetCw ();
          m_backoffTrace = (m_dcf->GetCw ()+1)/2;
          m_dcf->StartBackoffNow (m_backoffTrace);
          RestartAccessIfNeeded ();
        }
    }
  ...
  if (!NeedDataRetransmission (m_currentPacket, m_currentHdr))
  ...
  else
    {
      ...
      m_cwTrace = m_dcf->GetCw ();
        ...
        {
          m_backoffTrace = GetRandomNumber(0, m_dcf->GetCw());
        //std::cout<<m_dcf->GetCw()<<" "<<m_backoffTrace<<std::endl;
        }
  m_dcf->StartBackoffNow (m_backoffTrace);
  RestartAccessIfNeeded ();
}
...
{//std::cout<<"missedblockack "<<Simulator::Now().GetSeconds()<<std::endl;
  ...
  if ((m_currentPacket != 0
      ...
      && !m_dcf->IsAccessRequested ())
    {
      ...
      if (packet != 0)
		...
		/////liang
	//if(GetLow()->BusytoneIsIdle())
      m_manager->RequestAccess (m_dcf);
	//else
	  //Simulator::Schedule (MicroSeconds(30), &EdcaTxopN::RestartAccessIfNeeded, this);//////why 30 ms?
    }
}
...
{//std::cout<<"startnextpacket "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
...
{///std::cout<<"Gotblockack "<<Simulator::Now().GetSeconds()<<std::endl;
...
{////std::cout<<"SetupBlockAckIfNeeded"<<Simulator::Now().GetMicroSeconds()<<std::endl;
  NS_LOG_FUNCTION (this);
  uint8_t tid = m_currentHdr.GetQosTid ();
  Mac48Address recipient = m_currentHdr.GetAddr1 ();
...
{///////std::cout<<"sendblockackrequest "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  ...
  if (bar.immediate)
  ...
  else
    {
      ...
      m_currentParams.EnableAck ();
         ...
         // std::cout<<"blcokack "<<m_currentPacket->GetSize()<<std::endl;
  m_low->StartTransmission (m_currentPacket, &m_currentHdr, m_currentParams, this);
}
...
{//std::cout<<"sendaddbarequest "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  ...
  m_currentParams.DisableNextData ();
  m_currentParams.DisableOverrideDurationId ();
          //std::cout<<"addba "<<m_currentPacket->GetSize()<<std::endl;
  m_low->StartTransmission (m_currentPacket, &m_currentHdr, m_currentParams, this);
}
void /////liang
EdcaTxopN::ChannelBusy(void)
{//std::cout<<"busy2"<<std::endl;
    m_currentHdr.SetNoRetry();
    m_dcf->UpdateFailedCw ();
    m_dcf->StartBackoffNow (m_rng->GetInteger (0, m_dcf->GetCw ()));  //////從原本的stage不改變 再隨機選一個backoff time
    RestartAccessIfNeeded ();
}
...
{//std::cout<<"senddelbaframe "<<Simulator::Now().GetSeconds()<<std::endl;
