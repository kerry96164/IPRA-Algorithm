...
enum
{
  ...
  SUBTYPE_CTL_CTS = 12,
  SUBTYPE_CTL_ACK = 13,
  SUBTYPE_CTL_POWER =14,////steve
  SUBTYPE_CTL_DISTANCE = 15, // Jonathan
};
...
void
WifiMacHeader::Settoleratepower(uint16_t toleratepower)
{
  m_toleratepower = toleratepower;
}
uint16_t
WifiMacHeader::Gettolerate(void)
{
...
void
WifiMacHeader::Settxpower(uint16_t txpower)
{
  m_txpower = txpower;
}
uint16_t
...
void
WifiMacHeader::Setpowerboolean(uint16_t yorn)
{
  m_ifgreaterthanzero = yorn;
}
 bool
WifiMacHeader::ifisgreaterthanzero(void)
{
  if(double(m_ifgreaterthanzero)>0)
    return true;
  else
    return false;
}
...
WifiMacHeader::SetType (WifiMacType type)
{
  switch (type)
    ...
    case WIFI_MAC_CTL_ACK:
      ...
      break;
    case WIFI_MAC_CTL_POWER:
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_POWER;//steve
      break;
    case WIFI_MAC_CTL_DISTANCE:
      m_ctrlType = TYPE_CTL;
      m_ctrlSubtype = SUBTYPE_CTL_DISTANCE; // Jonathan
      break;      
  ...
  switch (m_ctrlType)
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        ...
        case SUBTYPE_CTL_POWER:
          return WIFI_MAC_CTL_POWER;
        case SUBTYPE_CTL_DISTANCE: // Jonathan
          return WIFI_MAC_CTL_DISTANCE;
        }
      break;
    case TYPE_DATA:
      switch (m_ctrlSubtype)
...
bool
WifiMacHeader::Ispowercontrol (void) const
{
  return (GetType () == WIFI_MAC_CTL_POWER);
}

bool // Jonathan
WifiMacHeader::IsPositionPacket (void) const
{
  return (GetType () == WIFI_MAC_CTL_DISTANCE);
}
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        ...
        case SUBTYPE_CTL_POWER:
          size =  2 + 2 + 2 + 2 + 6;
          break;
        case SUBTYPE_CTL_DISTANCE:
          size = 2/*Frame Control*/ + 8/*x*/ + 8/*y*/ + 8/*z*/ + 6/*Rx Mac address*/ + 6/*Tx Mac address*/; // Jonathan
          break;
        case SUBTYPE_CTL_CTS:
          size = 2  + 2 +6;
          break;
...
{
  i.WriteHtolsbU16 (GetFrameControl ()); // 2 bytes
  if(m_ctrlSubtype==SUBTYPE_CTL_POWER)
    {
      i.WriteHtolsbU16 (m_toleratepower);
      i.WriteHtolsbU16 (m_txpower);
      i.WriteHtolsbU16 (m_duration);
    }
  else if(m_ctrlSubtype==SUBTYPE_CTL_DISTANCE) // Jonathan
  {
      i.WriteHtolsbU64 (m_position.x); // 8 bytes
      i.WriteHtolsbU64 (m_position.y);
      i.WriteHtolsbU64 (m_position.z);
  }
  else if(m_ctrlType == TYPE_DATA)
    {
      i.WriteHtolsbU16 (m_txpower);
      i.WriteHtolsbU16 (m_duration);
    }
  else
    i.WriteHtolsbU16 (m_duration); // 2 bytes
  WriteTo (i, m_addr1); //RA 6 bytes
  switch (m_ctrlType)
    {
    case TYPE_MGT:
      WriteTo (i, m_addr2);
    ...
    case TYPE_CTL:
      switch (m_ctrlSubtype)
        {
        case SUBTYPE_CTL_RTS:
          WriteTo (i, m_addr2); // TA
          break;
        case SUBTYPE_CTL_POWER:
          break; // Jonathan
        case SUBTYPE_CTL_DISTANCE:
          WriteTo (i, m_addr2); // TA , by Jonathan
          break;
        case SUBTYPE_CTL_CTS:
        case SUBTYPE_CTL_ACK:
          break;
...
WifiMacHeader::Deserialize (Buffer::Iterator start)
{
  ...
  // std::cout << frame_control << std::endl; //Jonathan
  SetFrameControl (frame_control);
  if(m_ctrlSubtype==SUBTYPE_CTL_POWER)
    {
      m_toleratepower = i.ReadLsbtohU16 ();
      m_txpower = i.ReadLsbtohU16 ();
      m_duration = i.ReadLsbtohU16 ();
    }
  else if(m_ctrlSubtype==SUBTYPE_CTL_DISTANCE) // Jonathan
  {
    m_position.x = i.ReadLsbtohU64 ();
    m_position.y = i.ReadLsbtohU64 ();
    m_position.z = i.ReadLsbtohU64 ();
  }
  else if(m_ctrlType == TYPE_DATA)
    {
      m_txpower = i.ReadLsbtohU16 ();
